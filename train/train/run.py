...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    with open(os.path.join(artefacts_path, "params.json"), 'w') as json_file:
        json.dump(train_conf, json_file)

    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.json
    with open(os.path.join(artefacts_path, "labels_index.json"), 'w') as json_file:
        json.dump(dataset.get_label_to_index_map(), json_file)
