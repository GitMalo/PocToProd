...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        """
        # TODO: CODE HERE
        liste_label = self._get_label_list()
        dict = {}

        for index, label in enumerate(liste_label):
            dict[index] = label
    ...
    def to_indexes(self, labels):
        ...
        for label in labels:
            liste_index.append(self.get_label_to_index_map()[label])
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert list_columns == list(df.columns)
