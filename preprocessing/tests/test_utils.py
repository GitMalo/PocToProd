...
class TestBaseTextCategorizationDataset(unittest.TestCase):
    ...
    def test_index_to_label_and_label_to_index_are_identity(self):
        ...
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base._get_label_list = MagicMock(return_value=['label_a', 'label_b'])
        expected_map_1 = {0: 'label_a', 1: 'label_b'}
        self.assertEqual(base.get_index_to_label_map(), expected_map_1)
        expected_map_2 = {'label_a': 0, 'label_b': 1}
        self.assertEqual(base.get_label_to_index_map(), expected_map_2)
...
class TestLocalTextCategorizationDataset(unittest.TestCase):
    ...
    def test__get_num_samples_is_correct(self):
        ...
        cls = utils.LocalTextCategorizationDataset("fake_path", 1, min_samples_per_label=1)

        self.assertEqual(cls._get_num_samples(), 10)
    ...
    def test_get_train_batch_returns_expected_shape(self):
        ...
        cls = utils.LocalTextCategorizationDataset("fake_path", 1, min_samples_per_label=1)
        next_x, next_y = cls.get_train_batch()
        expected_shape_x = (1,)
        expected_shape_y = (1,2)
    ...
    def test_get_test_batch_returns_expected_shape(self):
        ...
        cls = utils.LocalTextCategorizationDataset("fake_path", 1, min_samples_per_label=1)
        next_x, next_y = cls.get_test_batch()
        expected_shape_x = (1,)
        expected_shape_y = (1, 2)
    ...
    def test_get_train_batch_raises_assertion_error(self):
        ...
        with self.assertRaises(AssertionError):
            cls = utils.LocalTextCategorizationDataset("fake_file", 7, train_ratio=0.5, min_samples_per_label=1)